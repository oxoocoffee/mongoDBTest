...
class CallTree{
	...
	static string GetPerformanceReport(const string& prefix){
		...
		for(unsigned int i = 0; i < GetSortedRootsCache().size(); ++i){
...
class Scope{
	...
	public: Scope(const string& text, FrameType type){
		...
		if(type == Section){
            ...
            while(frameIter != Stack::Get()->trace.rend() and
                    (*frameIter)->type != Function and
...
#define CONCAT(x, y) CONCATENATE_DETAIL(x, y)
    ...
    static string extractFileName(const string& path) __attribute__ ((unused));
